...
- Debug 构建默认使用文件凭据后端，避免开发签名反复触发系统凭据提示；可设置 `DEPLOYKEYS_CREDENTIALS_BACKEND=keychain` 强制使用系统凭据存储。
...
- Rust 1.75+
- macOS 10.15+、Linux x86_64/arm64 或 Windows 10/11 x64
- Git 和 OpenSSH 客户端
- `sqlite3` CLI，用于创建 SQLx 编译期检查数据库
- `wasm32-unknown-unknown` target
- Trunk
- Tauri CLI v2
- Windows 下使用内置 Tailwind 安装脚本或 `make` 目标时，需要 Git Bash/MSYS 这类 POSIX shell
...
Windows 下建议在 Git Bash/MSYS 中执行这些 shell 命令；如果使用 PowerShell，
请先把 Tailwind CLI 安装到 `tools/tailwindcss.exe`，再直接运行等价的
`cargo` 和 `trunk` 命令。
...
开发构建如果使用默认文件凭据后端，明文凭据会写入同一数据目录下的 `dev_credentials.json`。这是开发便利设置，发布构建默认走系统凭据存储。
